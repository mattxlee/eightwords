...
Convertor::Convertor(face in_face, face out_face) {
  _iconv = iconv_open(
      face_to_string(out_face).c_str(), face_to_string(in_face).c_str());
}
...
std::string Convertor::convert(std::string const& src) {
  char const* sz_src = src.c_str();
  ...
  char* sz_dst = sz_dst2;
  int err
      = iconv(_iconv, const_cast<char**>(&sz_src), &len_src, &sz_dst, &len_dst);
  if (err == -1) {
    delete[] sz_dst2;
