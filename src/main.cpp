...
int main(int argc, char const* argv[]) {
  try {
    ...
    CalendarDay chinaDay
        = calendar.queryChineseDay(args.year, args.month, args.day);

    // 查询时辰
    Table8x2 table;
    CNStringList result = table.analyze(
        chinaDay.chinaYear, chinaDay.chinaMonth, chinaDay.chinaDay, args.hour);

    // 输出八字
    for (CNString const& str : result) {
      for (CNChar const& ch : str) {
        std::cout << ch;
      }
      std::cout << " ";
    }
